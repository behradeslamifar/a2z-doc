##### راهنمای استفاده از مستندات
این مستند پر است از دستورات و نمونه کانفیگ هایی که در تجربه‌های مختلف در اینجا مستند شده‌اند. تمام دستورات و تنظیمات در داخل Code Block قرار گرفته اند. سعی کردم که استانداردی برای مطالعه بهتر داشته باشم. اولین موضوع اینکه اگر قبل از دستور $ بود، یعنی می توان با کاربر عادی دستور را اجرا کرد و اگر قبل از دستور # بود یعنی باید با کاربر root آنرا اجرا کنید.\\
در تنظیمات و دستورات بخش‌هایی که در داخل < > قرار دارند یعنی شما باید با مقدار خودتون جایگزین کنید و فیلد مورد نظر اجباری است. و اگر بخشی از دستور یا پیکربندی بین [ ] قرار داشته باشد، فیلد مورد نظر اختیاری است.\\


### A
  * [Ansible - Playbook Sample](ansible/sampleplaybook.md)
  * [Ansible - Use ansible and ansible-playbook](ansible/command.md)

### B
  * [Bash Scripting](bash/bashscripting.md)

### C
  * [CNCF CKA Curriculum](kubernetes/cka_curriculum.md)
### D
  * [Docker-compose - Commands](docker-compose/commands.md) 
  * [Docker-compose - Sample Config](docker-compose/sample.md) 

### E
  * [Elasticsearch API](elasticsearch/api.md)
  * [Elasticsearch Config Samples](elasticsearch/config.md)
  * [etcdctl usage](etcd/etcdctl.md)

### F
  * [fprintd - Howto](fprintd.md)

### G
  * [Git - Cheat Sheet](git/cheatsheet.md)

### J
  * [jq - Json Parser](jq.md)

### K
  * [Keepalived - HAProxy HA](keepalived/haproxy.md)
  * [kubectl - Cheat Sheet](kubectl/cheatsheet.md)
  * [Kubernetes - API Security](kubernetes/apisecurity.md)
  * [Kubernetes - Configmaps and Secrets](kubernetes/configmaps.md)
  * [Kubernetes - Expose Pod Access](kubernetes/exposepod.md)
  * [Kubernetes - Ingress](kubernetes/ingress.md)
  * [Kubernetes - Kubeconfig](kubernetes/kubeconfig.md)
  * [Kubernetes - Pod Controller](kubernetes/podcontroller.md)

### L
  * [Letsencrypt - Certbot](letsencrypt/certbot.md)

### M
  * [Mailman - Notes](mailman/notes.md)
  * [Minio - Installation](minio/installation.md) 
  * [Minio - User Management](minio/user_management.md)
  * [mmcli - Connect to Sim Card Internet](mmcli.md)

### O
  * [Opennebula - Create Window Template](opennebula/windowstemplate.md)

### P
  * [Pacemaker - Config Examples](pacemaker/configsample.md)
  * [Pacemaker - Quick Guide](pacemaker/quickguide.md)
  * [Pacemaker - Resource Agents](pacemaker/ra.md)
  * [Packer Howto](packer/howto.md)
  * [Postfix - Config with Postfixadmin](postfix/postfixadmin.md)
  * [Postfix - Minimal Config](postfix/minimalconfig.md)
  * [Postfix - Secure SMTP](postfix/tlsconfig.md)
  * [PostgreSQL - Cheat Sheet](postgresql/cheatsheet.md)

### R
  * [RabbitMQ Commands](rabbitmq/command.md)
  * [Redis - Config Notes](redis/note.md)

### S
  * [Sed - Tutorial](sed/tutorial.md) 
  * [Squid - Example Config 1](squid/anonymouse.md)
  * [Swarm - Cheat sheet](swarm/cheatsheet.md)
  * [Sysctl Config Sample](sysctl/sampleconfig.md)
